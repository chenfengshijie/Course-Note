...
3. 需要考虑隐藏终端问题
### CSMA/CA
CA 是碰撞避免，与CD碰撞检测相比，碰撞避免比在检测是否碰撞，而是在碰撞之后开始计时，每当信道空闲，计时器就会减一，当计时器为0就开始发送，如果还是碰撞就设定更大的计时器。

### RTS和CTS
为了解决隐藏终端的问题，移动设别通过发送一个很短的数据帧请求发送（Request to Send），服务器通过广播一个允许发送数据帧（Clear to Send)，这个时候其他的设别就休眠，当前就只有预约的设别发送数据。
